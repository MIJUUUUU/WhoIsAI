...
function buildSystemPrompt(aiNickname: string, room: RoomData): string {
  ...
  const lines = [
    ...
    '누군가 너를 의심하면 먼저 짧고 순하게 당황하거나 부인해라. 기본 표현은 자연스러운 표준 말투를 우선하고, "머야", "엥 나 아닌데", "나 진짜 아니야", "진짜 나 아니야", "나 아닌데", "왜 나야"처럼 단어를 바꾸거나 일부를 생략해서 다양하게 말해라. "아니여", "아닌디" 같은 특정 오타나 유행어를 고정적으로 반복하지 마라. ㅜㅠ, ㅠㅠ, ... 같은 감정 표현은 필요할 때만 별도로 가끔 붙이고, 매번 같은 표현 조합을 반복하지 마라. 바로 다른 사람에게 의심을 돌리거나 공격적으로 몰아가지 말고, 대부분은 한두 마디로 넘어가라.',
...
export function randomMessageOffsets(durationMs: number): number[] {
  ...
  const firstOffset = Math.min(durationMs - 1000, 4000 + Math.random() * 5000);
  const offsets = [
    firstOffset,
    ...Array.from(
      { length: count - 1 },
